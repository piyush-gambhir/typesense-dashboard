...
import { MessageSquare, Plus, Search, Trash2, Edit, Download, RotateCcw } from 'lucide-react';
...
export default function NLConversations({ 
  ...
  const [selectedCollection, setSelectedCollection] = useState<string>(collectionName || 'all');
  ...
  const fetchConversations = async () => {
    setLoading(true);
    try {
      const params = selectedCollection !== 'all' ? { collection_name: selectedCollection } : undefined;
  ...
  const confirmClearHistory = async () => {
    ...
    try {
      const result = await clearConversationHistory(conversationToClear.id);
      if (result.success) {
        toast({
          title: 'Success',
        ...
        toast({
          title: 'Error',
          description: result.error || 'Failed to clear conversation history.',
          variant: 'destructive',
        });
      }
    } catch (error) {
  ...
  const handleExport = async (conversation: Conversation, format: 'json' | 'csv' | 'txt') => {
    try {
      ...
      if (result.success && result.data) {
        ...
        const dataBlob = new Blob([dataStr], { type: getContentType(format) });
        ...
        toast({
          title: 'Error',
          description: result.error || 'Failed to export conversation.',
          variant: 'destructive',
        });
      }
    } catch (error) {
  ...
  const filteredConversations = conversations.filter(conversation =>
    ...
    conversation.collection_name.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const getUniqueCollections = () => {
    const collections = [...new Set(conversations.map(c => c.collection_name))];
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Manage your natural language search conversations and chat history
          </p>
        </div>
      ...
      {filteredConversations.length === 0 ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              View and manage your natural language search conversations
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              ...
              <TableBody>
                {filteredConversations.map((conversation) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Badge variant="outline">{conversation.collection_name}</Badge>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <span>{conversation.messages.length}</span>
                        {conversation.metadata?.total_searches && (
                          <Badge variant="secondary" className="text-xs">
                            {conversation.metadata.total_searches} searches
                          </Badge>
                        )}
                      </div>
                    </TableCell>
                    <TableCell>
                      {conversation.updated_at 
                        ? formatDistanceToNow(new Date(conversation.updated_at * 1000), { addSuffix: true })
                        : 'Unknown'
                      }
                    </TableCell>
                    <TableCell>
                      {conversation.model_id ? (
                        <Badge variant="outline">{conversation.model_id}</Badge>
                      ) : (
                    ...
                    <TableCell>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuSeparator />
                          <DropdownMenuItem onClick={() => handleExport(conversation, 'json')}>
                            <Download className="h-4 w-4 mr-2" />
                            Export JSON
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleExport(conversation, 'csv')}>
                            <Download className="h-4 w-4 mr-2" />
                            Export CSV
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleExport(conversation, 'txt')}>
                            <Download className="h-4 w-4 mr-2" />
                            Export Text
                          </DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem onClick={() => handleClearHistory(conversation)}>
                            <RotateCcw className="h-4 w-4 mr-2" />
                            Clear History
                          </DropdownMenuItem>
                          <DropdownMenuItem 
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete the conversation "{conversationToDelete?.title || conversationToDelete?.id}"? 
      ...
      <Dialog open={isClearHistoryDialogOpen} onOpenChange={setIsClearHistoryDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to clear the history for "{conversationToClear?.title || conversationToClear?.id}"? 
