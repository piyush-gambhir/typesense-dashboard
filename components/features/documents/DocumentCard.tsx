...
const ResultField: React.FC<ResultFieldProps> = ({ field, value }) => {
  ...
  return (
    <div className="mb-4">
      <span className="font-semibold text-sm md:text-base block mb-2">{field}</span>
      ...
      <Card className="w-full flex flex-col justify-between">
        <CardContent className="p-4 md:p-6">
          <div className="space-y-4">
            <div className="mb-4">
              <span className="font-semibold text-sm md:text-base block mb-2">ID</span>
              <div className="text-sm md:text-base break-all">{result.id}</div>
            </div>
            {sortedEntries.map(
              ([key, value]) =>
                key !== 'id' && (
