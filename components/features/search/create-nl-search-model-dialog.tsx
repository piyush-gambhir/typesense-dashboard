...
export default function CreateNLSearchModelDialog({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const payload = {
                name: formData.id,
                model_type: formData.model_name,
                model_config: {
                    model_name: formData.model_name,
                    api_key: formData.api_key,
                    system_prompt: formData.system_prompt,
                    max_bytes: formData.max_bytes,
                    temperature: formData.temperature,
                },
                collections: [], // You may want to allow user input for this
            };
            const result = await createNLSearchModel(payload);
            if (result.success) {
                toast({
                    title: 'Success',
                ...
                toast({
                    title: 'Error',
                    description:
                        result.error || 'Failed to create NL search model.',
                    variant: 'destructive',
                });
            }
        } catch {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Configure a new natural language search model to enable
                        AI-powered search.
                    </DialogDescription>
                </DialogHeader>
