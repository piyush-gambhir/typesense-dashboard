...
export async function multiSearch({
  ...
  try {
    if (!typesenseClient?.multiSearch?.perform) {
      console.error('Typesense client or multiSearch method is not available');
      return null;
    }
    ...
    const multiSearchRequests = searchQueries.map((query) => {
      ...
      if (query.highlightFullFields && query.highlightFullFields.trim() !== '') {
    ...
    if (!response || !response.results) {
      console.error('Invalid response from Typesense multiSearch:', response);
      return null;
    }
