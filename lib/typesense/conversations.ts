...
export async function createConversation(request: CreateConversationRequest) {
  try {
    const response = await (typesenseClient as any).apiCall.post('/conversations', request);
    return {
      success: true,
...
export async function listConversations(params?: {
  ...
  try {
    ...
    if (params?.collection_name) queryParams.set('collection_name', params.collection_name);
    if (params?.model_id) queryParams.set('model_id', params.model_id);
    if (params?.limit) queryParams.set('limit', params.limit.toString());
    if (params?.offset) queryParams.set('offset', params.offset.toString());

    const url = queryParams.toString() ? `/conversations?${queryParams.toString()}` : '/conversations';
...
export async function getConversation(conversationId: string) {
  try {
    const response = await (typesenseClient as any).apiCall.get(`/conversations/${conversationId}`);
    return {
      success: true,
      data: response as Conversation,
    };
...
export async function updateConversation(conversationId: string, updates: UpdateConversationRequest) {
  try {
    const response = await (typesenseClient as any).apiCall.put(`/conversations/${conversationId}`, updates);
    return {
      success: true,
      data: response as Conversation,
    };
...
export async function deleteConversation(conversationId: string) {
  try {
    await (typesenseClient as any).apiCall.delete(`/conversations/${conversationId}`);
    return {
      success: true,
    };
  } catch (error) {
...
export async function conversationSearch(request: ConversationSearchRequest): Promise<{ success: boolean; data?: ConversationSearchResponse; error?: string }> {
  try {
    const response = await (typesenseClient as any).apiCall.post(`/conversations/${request.conversation_id}/search`, {
...
export async function clearConversationHistory(conversationId: string) {
  try {
    await (typesenseClient as any).apiCall.delete(`/conversations/${conversationId}/messages`);
    return {
      success: true,
    };
  } catch (error) {
