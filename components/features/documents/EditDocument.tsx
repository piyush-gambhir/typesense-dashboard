...
export default function EditDocumentPage({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (doc && schemaResult && schemaResult.success && schemaResult.data) {
        ...
        toast({
          title: 'Error',
          description: schemaResult?.error || 'Failed to load document or collection schema.',
          variant: 'destructive',
        });
      }
    };
  ...
  async function onSubmit(values: Record<string, any>) {
    setIsLoading(true);
    try {
      const updatedDoc = await updateDocument(collectionName, documentId, values);
      if (updatedDoc) {
        toast({
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        {field.type === 'string' ? (
                          ...
                          <Input type="checkbox" {...formField} checked={formField.value} onChange={e => formField.onChange(e.target.checked)} />
                        ) : field.type === 'string[]' ? (
