...
export default function NLSearchWrapper({
    collectionName,
}: NLSearchWrapperProps) {
    const { toast } = useToast();

    // Search mode: 'traditional' or 'natural' or 'conversational'
    const [searchMode, setSearchMode] = useState<
        'traditional' | 'natural' | 'conversational'
    >('traditional');
    ...
    const loadAvailableModels = async () => {
        try {
            ...
            if (result.success && result.data) {
                ...
                const defaultModel = result.data.find((m: NLSearchModel) =>
    ...
    const performNaturalLanguageSearch = async () => {
        ...
        try {
            const result = await naturalLanguageSearch({
                ...
                page: nlCurrentPage,
                per_page: nlPerPage,
                nl_model_id: selectedModel || undefined,
                debug: debugMode,
            });
            ...
            if (result.success && result.data) {
                ...
                setNlTotalPages(
                    Math.ceil((result.data.found || 0) / nlPerPage),
                );
                        ...
                        <div className="flex gap-2">
                            ...
                            >
                                ...
                                <DialogContent>
                                    <DialogHeader>
                                        <DialogTitle>
                                            Natural Language Search Settings
                                        </DialogTitle>
                                        <DialogDescription>
                                            Configure your AI-powered search
                                            preferences
                                        </DialogDescription>
                                    </DialogHeader>
                        ...
                        <Alert className="mb-4">
                            <AlertDescription>
                                <div className="space-y-2 text-sm">
                                    ...
                                    {typeof debugInfo.parsed_query !==
                                        'undefined' && (
                                        <div>
                                            <strong>Parsed Query:</strong>{' '}
                                            {`${JSON.stringify(debugInfo.parsed_query ?? '')}`}
                                        </div>
                                    )}
                                    <div className="flex gap-4 text-xs text-muted-foreground">
                                        <span>
                                            Search Time:{' '}
                                            {`${JSON.stringify(debugInfo.search_time_ms ?? '')}`}
                                            ms
                                        </span>
                                        {typeof debugInfo.request_params !==
                                            'undefined' && (
                                            <span>
                                                Parameters:{' '}
                                                {`${JSON.stringify(debugInfo.request_params ?? '')}`}
                                            </span>
                                        )}
                                    </div>
                                </div>
                        ...
                        <div className="lg:col-span-4 flex flex-col gap-y-8">
                            <div className="flex flex-col sm:flex-row justify-end items-start sm:items-center mb-4 gap-4">
                                <Select
                                    ...
                                    onValueChange={(value) =>
                                        handleNlPerPageChange(parseInt(value))
                                    }
                                >
