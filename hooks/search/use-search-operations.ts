    ...
    const performSearch = useCallback(
        async (
            ...
            if (indexFields.length === 0) {
                console.warn(
                    '[performSearch] No string index fields available for search',
                );
            ...
            const validFilters = filterBy.filter((filter) => {
                ...
                if (!filter.includes(':=')) {
                    console.warn(
                        '[performSearch] Invalid filter format:',
                        filter,
                    );
                    return false;
                }
            ...
            try {
                ...
                if (
                    ...
                    if (documentsResponse) {
                        ...
                        const typedResponse =
                            documentsResponse as SearchResponse;
                    ...
                    console.error(
                        '[performSearch] response.results is missing or empty:',
                        response?.results,
                    );
                    setSearchResults([]);
                    setTotalResults(0);
